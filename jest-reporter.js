...
class RussianReporter extends DefaultReporter {
  ...
  getHeader() {
    return `${this.colors.cyan}${this.colors.bright}═══════════════════════════════════════════════════════════${this.colors.reset}\n` +
           `${this.colors.cyan}${this.colors.bright}  🧪 Запуск тестов QuickShare${this.colors.reset}\n` +
           `${this.colors.cyan}${this.colors.bright}═══════════════════════════════════════════════════════════${this.colors.reset}\n`;
  }

  // Форматирование результата теста
  formatTestResult(test, indent = '  ') {
    const status = this.getStatusText(test.status);
    const duration = test.duration ? ` (${test.duration}ms)` : '';
    const title = test.title;
    
    return `${indent}${status} ${this.colors.bright}${title}${this.colors.reset}${this.colors.gray}${duration}${this.colors.reset}`;
  }

  // Форматирование описания набора тестов
  formatTestSuite(suite, indent = '') {
    let output = `${indent}${this.colors.blue}${this.colors.bright}${suite.title}${this.colors.reset}\n`;
    
    if (suite.tests) {
      suite.tests.forEach(test => {
        output += this.formatTestResult(test, indent + '  ') + '\n';
      });
    }
    
    if (suite.children) {
      suite.children.forEach(child => {
        output += this.formatTestSuite(child, indent + '  ');
      });
    }
