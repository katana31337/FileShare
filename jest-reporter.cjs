...
class RussianReporter extends DefaultReporter {
  ...
  onRunComplete(contexts, results) {
    ...
    console.log(`${this.colors.cyan}${this.colors.bright}  📊 ИТОГИ ТЕСТИРОВАНИЯ FRONTEND${this.colors.reset}`);
    console.log(`${this.colors.cyan}${'═'.repeat(70)}${this.colors.reset}\n`);
    ...
    if (failed.length === 0) {
      console.log(`\n  ${this.colors.green}${this.colors.bright}🎉 ВСЕ ТЕСТЫ FRONTEND УСПЕШНО ПРОЙДЕНЫ! 🎉${this.colors.reset}\n`);
    } else {
      console.log(`\n  ${this.colors.red}${this.colors.bright}⚠️  ЕСТЬ ПРОВАЛЕННЫЕ ТЕСТЫ FRONTEND - ПРОВЕРЬТЕ ВЫШЕ${this.colors.reset}\n`);
    }
